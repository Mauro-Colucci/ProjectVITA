...
export const getBookmarks = async (req, res) => {
  try {
    const myBookmarks = await Project.find({
      bookmarks: { $in: req.user.id },
    });
    res.render("projects", {
...
export const getFeed = async (req, res) => {
  try {
    ...
    const projects = await Project.find({ publish: { $ne: false } }) //find all documents with publish not equal to false
