...
export const getOwnTask = async (req, res) => {
  let singleTask = null;
  let comments = null;
  const id = "";
  try {
    ...
    if (req.params.myTasksId) {
      singleTask = await Task.findById(req.params.myTasksId)
        .populate("projectId createdBy assignedTo")
        .lean();
      comments = await Comment.find({ taskId: req.params.myTasksId })
        .sort({ createdAt: "desc" })
        .populate("createdBy")
        .lean();
    }
    ...
    res.render("taskPage", {
      ...
      showSearch: true,
      loggedUser: req.user,
      userIsAdmin: reqUserIsAdmin.isAdmin,
      id,
      myTask: true,
    });
  } catch (err) {
...
export const getProjectTask = async (req, res) => {
  ...
  let comments = null;
  try {
    const projectAllTasks = await Task.find({
    ...
    if (req.params.taskId) {
      singleTask = await Task.findById(req.params.taskId)
        .populate("projectId createdBy assignedTo")
        .lean();
      comments = await Comment.find({ taskId: req.params.taskId })
        .sort({ createdAt: "desc" })
        .populate("createdBy")
        .lean();
    }
...
export const deleteTask = async (req, res) => {
  ...
  try {
    ...
    const redirect = req.headers.referer.split("/").includes("singleTask" && id)
      ? `/task/${task.projectId}`
      : "back";
